#version 300 es
// #version must be the literal first line in the ES profile.
//
// sarphiv's in-game CRT shader, ported from Ghostty's Shadertoy-style
// interface to Hyprland's. The body below is unmodified apart from
// BACKGROUND_OPACITY; the shims map iChannel0/iTime/iResolution onto
// Hyprland's tex/time/fullSize.
...
#define iResolution vec3(fullSize, 1.0)
...
// x \in R : x >= 0
#define VIGNETTE_BRIGHTNESS 2.26
...
// [0, 1]
#define SCAN_LINES_STRENGTH 0.12
...
// [0, 1]
#define NOISE_CONTENT_STRENGTH 0.12
// How much noise is added everywhere
// [0, 1]
#define NOISE_UNIFORM_STRENGTH 0.12
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Add flicker
    fragColor *= 1.0 - FLICKER_STRENGTH/2.0*(1.0 + sin(2.0*PI*FLICKER_FREQUENCY*0.0));


    // Add noise
    // NOTE: Hard-coded noise distributions
    float noise = smoothstep(0.4, 0.6, gold_v2_noise(fragCoord.xy, 0.0));
...
void main() {
    vec4 outColor;
    mainImage(outColor, v_texcoord * fullSize);
    fragColor = outColor;
}
